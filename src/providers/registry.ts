 ...
 * PACKDEV_PROVIDER_MODULE must resolve against the CALLER's working directory
...
/**
 * Resolves which forge this run talks to. PACKDEV_PROVIDER_MODULE, if set, always
 * wins regardless of PACKDEV_PROVIDER — it's the third-party extensibility path:
 ...
 * Otherwise PACKDEV_PROVIDER selects a built-in ("github" by default, "gitea"
 * also available today).
 */
export async function resolveProvider(env: NodeJS.ProcessEnv): Promise<Provider> {
  const moduleSpecifier = env["PACKDEV_PROVIDER_MODULE"];
  if (moduleSpecifier) {
    ...
    if (typeof factory !== "function") {
      throw new Error(
        `PACKDEV_PROVIDER_MODULE "${moduleSpecifier}" must have a default export that is a function (env) => Provider — see src/providers/types.ts's ProviderFactory.`,
      );
    }
  ...
  if (!factory) {
    ...
    throw new Error(
      `Unknown PACKDEV_PROVIDER "${providerName}" — expected one of: ${known} (or set PACKDEV_PROVIDER_MODULE to a custom provider).`,
    );
  }
  return factory(env);
}
