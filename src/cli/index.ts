...
function buildBrain(): Brain | undefined {
  const kind = env("PACKDEV_BRAIN") ?? "none";
  ...
  switch (kind) {
    ...
    default:
      throw new Error(`Unknown PACKDEV_BRAIN "${kind}" — expected "none", "anthropic", or "openai-compatible"`);
  }
}

/** Mirrors buildBrain()'s shape — same "one interface, pick a backend by env" pattern, distinct env var (PACKDEV_MODEL_PROVIDER) since triage's model choice is independent of compat's optional failure-summary brain. */
function buildAgentLoop(): AgentLoop {
  const provider = env("PACKDEV_MODEL_PROVIDER") ?? "anthropic";
  ...
  switch (provider) {
    ...
    default:
      throw new Error(`Unknown PACKDEV_MODEL_PROVIDER "${provider}" — expected "anthropic" or "openai-compatible"`);
  }
}
 ...
 * no PACKDEV_ALLOWED_ACTORS override. Only applied when PACKDEV_ALLOWED_ACTORS is unset
 * AND PACKDEV_PROVIDER=gitea — an explicit PACKDEV_ALLOWED_ACTORS always wins, and a
 * PACKDEV_PROVIDER_MODULE's own actor-naming convention is unknowable here, so it
...
/** Shared by both subcommands — no test-execution config here, see readTestConfig(), which only `compat` actually needs. PACKDEV_CLONE_DIR/PACKDEV_STATE_PATH are resolved per repo, see resolveRepoPaths(). */
export function readCommonEnv() {
  const allowedActorsInput = env("PACKDEV_ALLOWED_ACTORS");
  const allowedActors = allowedActorsInput
    ? allowedActorsInput.split(",").map((s) => s.trim()).filter(Boolean)
    : env("PACKDEV_PROVIDER") === "gitea"
      ? GITEA_DEFAULT_ALLOWED_ACTORS
      : undefined;

  return {
    allowedActors,
    packageJsonPath: env("PACKDEV_PACKAGE_JSON_PATH"),
  };
}

/**
 * PACKDEV_REPO ("owner/repo") is the single-repo form; PACKDEV_REPOS ("owner/a,owner/b")
 * watches a whole list with the same PACKDEV_PROVIDER/token — the common case for
 * a bot account with access to several repos in one org. Mutually
 * exclusive with PACKDEV_REPO so there's exactly one way to say "one repo".
 */
export function readRepoList(): string[] {
  const repoSingle = env("PACKDEV_REPO");
  const reposList = env("PACKDEV_REPOS");
  if (repoSingle && reposList) {
    throw new Error("PACKDEV_REPO and PACKDEV_REPOS are mutually exclusive, got both.");
  }
  if (reposList !== undefined) {
    const repos = reposList.split(",").map((s) => s.trim()).filter(Boolean);
    if (repos.length === 0) {
      throw new Error(`PACKDEV_REPOS must contain at least one "owner/repo", got "${reposList}".`);
    }
    return repos;
  }
  if (repoSingle) return [repoSingle];
  throw new Error("Missing required environment variable: PACKDEV_REPO (or PACKDEV_REPOS for a comma-separated list)");
}
 ...
 * onto the same segment, which would make those two repos in a PACKDEV_REPOS list
...
/**
 * A single repo (PACKDEV_REPO, or PACKDEV_REPOS with exactly one entry) keeps today's flat
 ...
 *
 * A PACKDEV_REPOS list namespaces PACKDEV_CLONE_DIR/PACKDEV_STATE_PATH into a root dir with one
 ...
 * need a meaningless dummy PACKDEV_TEST_COMMAND just to pass a check it never uses.
 */
function readTestConfig(): { testCommand: string | undefined; testScript: string | undefined } {
  const testCommand = env("PACKDEV_TEST_COMMAND");
  const testScript = env("PACKDEV_TEST_SCRIPT");
  if (!testCommand && !testScript) {
    throw new Error("Exactly one of PACKDEV_TEST_COMMAND/PACKDEV_TEST_SCRIPT env vars is required, got neither.");
  }
  if (testCommand && testScript) {
    throw new Error("PACKDEV_TEST_COMMAND and PACKDEV_TEST_SCRIPT are mutually exclusive, got both.");
  }
  return { testCommand, testScript };
}

/**
 * PACKDEV_REMOTE_URL is an optional override — by default the git remote (and its
 ...
 * itself. The authHeader is only forwarded when PACKDEV_REMOTE_URL is unset or
 * points at the same origin as the provider's own URL — otherwise a
 * PACKDEV_REMOTE_URL pointed at a different host would leak the forge token to
...
export function resolveGitRemote(provider: { createGitRemote(): { url: string; authHeader?: string | undefined } } ): { remoteUrl: string; authHeader: string | undefined } {
  ...
  const override = env("PACKDEV_REMOTE_URL");
  if (!override) {
...
/**
 * Resolves (and thereby validates — PACKDEV_REPO shape, required token/URL env
 * vars) every repo's provider up front, before any PACKDEV_TEST_COMMAND check or
 * actual poll work runs. A bad PACKDEV_REPO/token is a shared-config problem, not
...
export async function resolveProvidersForRepos(repos: string[]): Promise<Map<string, Provider>> {
  ...
  for (const repo of repos) {
    providers.set(repo, await resolveProvider({ ...process.env, PACKDEV_REPO: repo }));
  }
  return providers;
}
...
async function runCompatOnce(): Promise<RepoRunOutcome<PollResult>[]> {
  ...
  if (repos.length > 1 && env("PACKDEV_REMOTE_URL")) {
    throw new Error("PACKDEV_REMOTE_URL cannot be used with multiple PACKDEV_REPOS — it would point every repo's clone at the same git remote.");
  }
  ...
  const autoMerge = env("PACKDEV_AUTO_MERGE") === "true";
  const testCombinedBumpInput = env("PACKDEV_TEST_COMBINED_BUMP");
  if (testCombinedBumpInput !== undefined && testCombinedBumpInput !== "true" && testCombinedBumpInput !== "false") {
    throw new Error(`PACKDEV_TEST_COMBINED_BUMP must be "true" or "false", got "${testCombinedBumpInput}"`);
  }
  ...
  const paths = resolveRepoPaths(
    repos,
    env("PACKDEV_CLONE_DIR"),
    env("PACKDEV_STATE_PATH"),
...
function readMaxTurns(): number | undefined {
  const input = env("PACKDEV_MAX_TURNS");
  if (input === undefined) return undefined;
  const value = Number(input);
  if (!Number.isFinite(value) || !Number.isInteger(value) || value <= 0) {
    throw new Error(`PACKDEV_MAX_TURNS must be a positive integer, got "${input}"`);
  }
  return value;
}
...
function readWebhookConfig(): { port: number; path: string } {
  const portInput = env("PACKDEV_WEBHOOK_PORT") ?? "8080";
  const port = Number(portInput);
  if (!Number.isFinite(port) || !Number.isInteger(port) || port <= 0) {
    throw new Error(`PACKDEV_WEBHOOK_PORT must be a positive integer, got "${portInput}"`);
  }
  const webhookPath = env("PACKDEV_WEBHOOK_PATH") ?? "/webhook";
  return { port, path: webhookPath };
}

/**
 * Fails fast, before the server ever binds a port, rather than accepting
 * every request as unverified at runtime. A PACKDEV_PROVIDER_MODULE that doesn't
...
function assertWebhookReady(repos: string[], providers: Map<string, Provider>): void {
  ...
  if (env("PACKDEV_PROVIDER_MODULE")) return;
  const providerName = env("PACKDEV_PROVIDER") ?? "github";
  if (providerName === "github") requireEnv("PACKDEV_PROVIDER_WEBHOOK_SECRET");
  else if (providerName === "gitea") requireEnv("PACKDEV_PROVIDER_WEBHOOK_SECRET");
}
...
Env vars (both subcommands):
  PACKDEV_REPO              "owner/repo" — one repo
  PACKDEV_REPOS             "owner/a,owner/b,..." — a list, same PACKDEV_PROVIDER/token for all
                    (mutually exclusive with PACKDEV_REPO; each repo runs independently,
                    one repo failing doesn't stop the rest)
  PACKDEV_REMOTE_URL        optional override for the git clone URL, single-PACKDEV_REPO only —
                    by default this is derived from PACKDEV_PROVIDER/PACKDEV_REPO with credentials
                    applied per-request, never embedded in the URL itself
                    (see docs/self-hosted.md)
  PACKDEV_PROVIDER          "github" (default) or "gitea"
  PACKDEV_PROVIDER_MODULE   path (relative to cwd) or package specifier for a custom
                    provider module (overrides PACKDEV_PROVIDER) — see docs/self-hosted.md
  PACKDEV_PROVIDER_TOKEN    required for every built-in provider
  PACKDEV_PROVIDER_URL, PACKDEV_PROVIDER_USERNAME   additionally required when PACKDEV_PROVIDER=gitea
  PACKDEV_ALLOWED_ACTORS    optional, default "dependabot[bot],renovate[bot]"
                    (PACKDEV_PROVIDER=gitea also allows "renovate" — Gitea's actor
                    field has no [bot] suffix convention)
  PACKDEV_PACKAGE_JSON_PATH, PACKDEV_POLL_INTERVAL_SECONDS   optional
                    (PACKDEV_POLL_INTERVAL_SECONDS must be a positive number; only used without --once)
  PACKDEV_CLONE_DIR         optional — one repo: the clone dir itself (default ./.packdev-agents/repo).
                    PACKDEV_REPOS list: the root dir, namespaced one subdir per repo
                    (default ./.packdev-agents/repos)

compat-only:
  PACKDEV_TEST_COMMAND | PACKDEV_TEST_SCRIPT   exactly one required
  PACKDEV_STATE_PATH        optional — one repo: the state file itself (default ./.packdev-agents/state.json).
                    PACKDEV_REPOS list: the root dir, one file per repo (default ./.packdev-agents/state)
  PACKDEV_AUTO_MERGE, PACKDEV_TEST_COMBINED_BUMP
  PACKDEV_BRAIN=anthropic|openai-compatible + ANTHROPIC_*/OPENAI_COMPATIBLE_*   optional failure-summary prose

triage-only:
  PACKDEV_TRIAGE_STATE_PATH   same one-repo-vs-PACKDEV_REPOS-list shape as PACKDEV_STATE_PATH above
                       (default ./.packdev-agents/triage-state.json or .../triage-state)
  PACKDEV_MAX_TURNS
  PACKDEV_MODEL_PROVIDER=anthropic|openai-compatible (default anthropic) + ANTHROPIC_*/OPENAI_COMPATIBLE_*

--webhook mode (instead of the poll loop or --once):
  PACKDEV_WEBHOOK_PORT      optional, default 8080
  PACKDEV_WEBHOOK_PATH      optional, default /webhook
  PACKDEV_PROVIDER_WEBHOOK_SECRET   required for every built-in provider
  A PACKDEV_PROVIDER_MODULE must implement verifyWebhookSignature to be used here.
...
async function main(): Promise<void> {
  ...
  if (webhook) {
    ...
    await server.start();
    console.log(`[webhook] listening on port ${env("PACKDEV_WEBHOOK_PORT") ?? "8080"}, path ${env("PACKDEV_WEBHOOK_PATH") ?? "/webhook"}`);
    return;
  }
  ...
  const intervalSecondsInput = env("PACKDEV_POLL_INTERVAL_SECONDS") ?? "300";
  ...
  if (!Number.isFinite(intervalSeconds) || intervalSeconds <= 0) {
    ...
    throw new Error(`PACKDEV_POLL_INTERVAL_SECONDS must be a positive number, got "${intervalSecondsInput}"`);
  }
