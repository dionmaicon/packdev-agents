...
test("createGiteaProvider: PACKDEV_PROVIDER_URL without a trailing slash -> clean clone URL", () => {
...
test("createGiteaProvider: PACKDEV_PROVIDER_URL WITH a trailing slash -> normalized, no double slash", () => {
  const provider = createGiteaProvider(baseEnv({ PACKDEV_PROVIDER_URL: "https://gitea.example.com/" }));
  assert.equal(provider.createGitRemote().url, "https://gitea.example.com/owner/repo.git");
});

test("createGiteaProvider: PACKDEV_PROVIDER_URL with MULTIPLE trailing slashes -> normalized", () => {
  const provider = createGiteaProvider(baseEnv({ PACKDEV_PROVIDER_URL: "https://gitea.example.com///" }));
  assert.equal(provider.createGitRemote().url, "https://gitea.example.com/owner/repo.git");
});
...
test("verifyWebhookSignature: valid signature -> true", () => {
  const provider = createGiteaProvider(baseEnv({ PACKDEV_PROVIDER_WEBHOOK_SECRET: "s3cret" }));
